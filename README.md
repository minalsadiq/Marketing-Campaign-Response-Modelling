...
`requirements.txt` deliberately contains **only** what the deployed app
needs (`streamlit`, `scikit-learn`, `pandas`, `numpy`, `joblib`) -- not the
notebook-only packages (`matplotlib`, `seaborn`, `plotly`, `xgboost`,
`lightgbm`, `shap`, `optuna`), which live in `requirements-notebooks.txt`
instead. Community Cloud installs whatever's in the root
`requirements.txt` to build the app, so an unused, hard-to-build package
in that file (e.g. an old Pillow pulled in transitively by matplotlib) is
enough to fail the whole deploy even though the app itself never imports
it.

`scikit-learn` is pinned to the exact version the committed model
artifacts were saved with (see `models/README.md`) -- if you ever retrain
and re-save `final_model.joblib` / `final_preprocessor.joblib` with a
different scikit-learn version, update this pin to match.

**Python version:** Community Cloud does **not** currently honor a
`runtime.txt` or `.python-version` file for selecting the Python version --
the only supported control is the **Advanced settings** dialog when you
create or edit the deployment. Set it explicitly (3.11 or 3.12 are safe,
well-supported choices) rather than leaving it on whatever the platform
defaults to, since very new Python releases often don't have prebuilt
wheels yet for some packages, forcing a slow (and sometimes failing)
from-source build. If the build logs show a different Python version than
what you selected, that's a known platform-side bug, not a project
misconfiguration -- delete and redeploy the app, reselecting the version
