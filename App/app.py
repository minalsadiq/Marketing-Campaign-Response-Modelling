...
Business workflow:
    1. Enter one customer's campaign/profile information (raw values,
       exactly as they'd come from the bank's own systems).
    2. `predict.score_batch` applies the same feature engineering used
       during training (via `src.features.engineer_features` -- the
       single shared implementation, not a re-typed copy of it).
    ...
    5. The result is displayed as an easy-to-understand business decision-
       support output.
...
Note: this app used to build the engineered feature row itself
(month/day cyclical encoding, education ordinal map, age bucket, etc.)
in a function defined right here. That duplicated the logic already
living in src/features.py, which is exactly the train/serve-skew risk
this project flagged in Notebook 09 -- two implementations of the same
transformation can quietly drift apart. That duplicate function has been
removed; this file now only collects the raw form inputs and hands them
to `predict.score_batch`, which engineers the features itself.
"""
...
# --------------------------------------------------------------------------
# Helper functions (pure logic lives in app_logic.py so it can be unit
# tested without a Streamlit runtime; imported here for use in the UI)
# --------------------------------------------------------------------------

from app_logic import (  # noqa: E402
    MONTH_LABEL_TO_RAW,
    DAY_LABEL_TO_RAW,
    build_raw_customer_row,
    predict_customer,
    probability_label,
)
...
with st.sidebar:
    ...
    st.caption(
        "- Validation PR-AUC is ~0.20 (vs. ~0.11 for a random baseline) -- "
        "a real but modest ranking signal, not a highly accurate classifier.\n"
        "- Not probability-calibrated -- use the score for ranking/prioritizing, "
        "not as a literal probability.\n"
        "- Trained mostly on a low-conversion period; accuracy can degrade in "
        "higher-conversion regimes (see model_card.json)."
    )
...
with col3:
    ...
    poutcome = st.selectbox(
        ...
        help=(
            "Outcome of the previous marketing campaign for this customer. "
            "Matches the three categories the model was trained on."
        ),
    )
...
with col1:
    month_label = st.selectbox(
        "Campaign Month",
        list(MONTH_LABEL_TO_RAW.keys()),
        index=4,
    )

    day_label = st.selectbox(
        "Campaign Day",
        list(DAY_LABEL_TO_RAW.keys()),
        index=2,
    )
...
if predict_button:
    ...
    try:
        customer_df = build_raw_customer_row(
            age=age,
            job=job,
            marital=marital,
            education=education,
        ...
        # ------------------------------------------------------------------
        # Raw input + what the model actually saw
        # ------------------------------------------------------------------

        with st.expander("View submitted raw customer data"):
            raw_display = customer_df.T.reset_index()
            raw_display.columns = ["Field", "Value"]
            ...
            st.caption(
                "This is the raw form input, in the same schema as the "
                "bank's export. Feature engineering (age group, cyclical "
                "month/day encoding, education ordinal, etc.) happens "
                "inside `predict.score_batch`, not in this app."
            )
        ...
        st.markdown(
            ...
            performance variation across different time periods -- see
            reports/model_card.json for the full set of known limitations.
            </div>
            """,
            unsafe_allow_html=True,
        )
